...
bool AlgorithmDijkstras2::pathfind(Environment* env, const Position& start, const Position& end, std::vector<Position>& outPath)
{
	...
	while (!openSetNodes.empty())
	{
		...
		for (int i = 0; i < 6; i++)
		{
			...
			if (env->canVisit(neighbourPos))
			{
				...
				{
					float cost = currentNode->cost + currentNode->pos.distanceTo(neighbourPos);
					...
					if (iterator == openSet.end())
					{
						...
						neighbour = new Node(neighbourPos, cost, currentNode);
					...
					else
					{
						...
						//Only update the node if it would get a lesser cost.
